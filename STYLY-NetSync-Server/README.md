...
- v5 adds an optional `ReferenceFrameLocal` pose flag. Bound avatars send head, hands, and virtual transforms in the registered reference frame's local coordinates, and reuse the existing 8-byte physical slot as direct physical position/yaw.
...
- Protocol v5 position quantization ranges:
  ...
  - Direct physical payload for reference-frame-local poses (`physical`, 4×`int16`: `x, y, z, yaw`): `0.01 m` per unit for translation, `0.1°` for yaw.
...
Assumed unbound baseline (`protocolVersion=5`, `ReferenceFrameLocal` off):
