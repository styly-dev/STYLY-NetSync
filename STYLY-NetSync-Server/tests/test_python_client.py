...
import time
import threading
from styly_netsync import net_sync_manager, transform_data, client_transform_data, room_transform_data, NetSyncServer
...
def test_packaging():
    ...
    from styly_netsync import net_sync_manager, transform_data, client_transform_data, room_transform_data
...
def test_connectivity():
    ...
    server = NetSyncServer(dealer_port=5555, pub_port=5556, enable_beacon=False)
...
def test_transforms_pull_based():
    ...
    server = NetSyncServer(dealer_port=5557, pub_port=5558, enable_beacon=False)
    server_thread = threading.Thread(target=lambda: server.start(), daemon=True)
    server_thread.start()
    time.sleep(0.5)
...
def test_device_mapping():
    ...
    server = NetSyncServer(dealer_port=5559, pub_port=5560, enable_beacon=False)
    server_thread = threading.Thread(target=lambda: server.start(), daemon=True)
    server_thread.start()
    time.sleep(0.5)
    ...
    try:
        ...
        reverse_device_id = manager.get_device_id_from_client_no(client_no) if client_no else None
...
def test_rpc():
    ...
    server = NetSyncServer(dealer_port=5561, pub_port=5562, enable_beacon=False)
    server_thread = threading.Thread(target=lambda: server.start(), daemon=True)
    server_thread.start()
    time.sleep(0.5)
...
def test_network_variables():
    ...
    server = NetSyncServer(dealer_port=5563, pub_port=5564, enable_beacon=False)
    server_thread = threading.Thread(target=lambda: server.start(), daemon=True)
    server_thread.start()
    time.sleep(0.5)
    ...
    try:
        ...
        success2 = client1.set_client_variable(client2.client_no if client2.client_no else 1, "score", "100")
        ...
        score = client2.get_client_variable(client2.client_no if client2.client_no else 1, "score", "0")
...
def test_stealth_mode():
    ...
    server = NetSyncServer(dealer_port=5565, pub_port=5566, enable_beacon=False)
    server_thread = threading.Thread(target=lambda: server.start(), daemon=True)
    server_thread.start()
    time.sleep(0.5)
