...
MSG_GROUP_TRANSFORM = 2  # Group transform with short IDs only
...
def _serialize_client_data(buffer: bytearray, client: Dict[str, Any]) -> None:
    """Serialize a single client's data (shared by group transform and client transform)"""
    # Device ID
...
def serialize_group_transform(data: Dict[str, Any]) -> bytes:
    """Serialize group transform data with short IDs (2 bytes per client ID)
    
    Args:
        data: The group transform data with clientNo field in each client
    """
    buffer = bytearray()
    
    # Message type
    buffer.append(MSG_GROUP_TRANSFORM)
    
    # Group ID
    _pack_string(buffer, data.get('groupId', ''))
...
def deserialize(data: bytes) -> Tuple[int, Union[Dict[str, Any], None], bytes]:
    ...
    try:
        ...
        elif message_type == MSG_GROUP_TRANSFORM:
            return message_type, _deserialize_group_transform(data, offset), b''
...
def _deserialize_group_transform(data: bytes, offset: int) -> Dict[str, Any]:
    """Deserialize group transform with client numbers only"""
    result = {}
    
    # Group ID
    result['groupId'], offset = _unpack_string(data, offset)
