...
- Let Unity generate `.meta` files — never author or edit them by hand or with an LLM.
- They **are** tracked in this repo: commit each `.meta` together with its corresponding new, renamed, or moved asset/script. A new/moved asset missing its `.meta` (or an orphaned `.meta`) is a defect.
